...
ylabel = round(330);
k = 2;
[img_mask] = myBinaryMask(bird, xlabel, ylabel, k);
foreground = img_mask.*bird;
background = (1-img_mask).*bird;
D = bwdist(rgb2gray(img_mask));
[img] = mySpatiallyVaryingKernel(bird, d_thres, img_mask);
% disp(img);
% img_mask = double(img_mask)/255;

% chan = size(img, 3);
% for i=1:chan
%    foreground(:,:,i) = immultiply(img_mask(:,:,i), double(bird(:,:,i)));
%    background(:,:,i) = immultiply(double(bg_mask(:,:,i)), double(bird(:,:,i))/255);
% end
